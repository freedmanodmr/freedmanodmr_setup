...
#------------------------------------------------------------------------------
    ...
    def odmr_initialisation_optimisation_ps82(
             self,
             dataset: str,
             iterations: int,
             ...
             binwidth_ns: int = 1000,
             n_bins: int = 1000,
             detector_delay_ns: int = 0, # as measured on the TimeTagger
             integration_time: float = 1,   # time for accumulations of pulse sequences
             event_divider: int = 1,  # Reduce the number of events counted
             ):
         """
         ...
         with MyInstrumentManager() as mgr, DataSource(dataset) as data:
             ...
             for i in range(iterations):
                 ...
                 t, counts = tt20.run_histogram(
                     click_channel=1,     # detector
                     start_channel=5,     # PS82 trigger / sync
                     binwidth_ps=binwidth_ns * 1e3,
                     n_bins=n_bins,
                     capture_time_s=integration_time,
                     start_delay=0,
...
#------------------------------------------------------------------------------
    ...
    def odmr_sweep_linear_timetagger_awg(self,
                   dataset: str,
                   start_freq: float,
                   stop_freq: float,
                   num_points: int,
    ...
    def spin_echo_detected_freq_sweep_awg(
            self,
            dataset: str,
...
#------------------------------------------------------------------------------
    ...
    def odmr_initialization_measurement_awg(
            self,
            dataset: str,
            pulse_width: float,
    ...
    def odmr_readout_optimisation_awg(
             self,
             dataset: str,
             iterations: int,
    ...
    def optimize_readout_delay_awg(
                self,
                dataset: str,
                freq_hz: float,
...
#------------------------------------------------------------------------------
    ...
    def ce_odmr_initialisation_optimisation_ps82(
        self,
        dataset: str,
        iterations: int,
        ...
        binwidth_ns: int = 1000,
        n_bins: int = 1000,
        detector_delay_ns: int = 0,
        integration_time: float = 1,
        event_divider: int = 1,
        ):
        """
        Initialisation pulse optimisation using PulseStreamer + TimeTagger.
        
        Sequence:
            Laser: trigger → repeated init pulses → recovery
            Trigger: sync pulse → matching timing → recovery
            
        Output:
            Histogram traces streamed per iteration
        """
        ...
        with MyInstrumentManager() as mgr, DataSource(dataset) as data:
            ...
            # Laser ON/OFF train
            init_train = [(init_ns, 1), (pulse_gap, 0)] * number_of_pulses

            # Trigger version (all LOW, just timing match)
            init_train_trig = [(init_ns, 0), (pulse_gap, 0)] * number_of_pulses

            # ------------------------------------------------
            # Build full sequences (FLAT — critical fix)
            # ------------------------------------------------
            laser_seq = [(100, 1)] + init_train + [(recovery_ns, 0)]
            trig_seq  = [(100, 1)] + init_train_trig + [(recovery_ns, 0)]
            
            # ------------------------------------------------
            # Load into PulseStreamer
            # ------------------------------------------------
            ps82.channel_sequences = {}  # clear previous sequences
