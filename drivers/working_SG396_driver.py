...
class Modulation(_SG396):
    ...
    def set_mod_frequency(self, freq_hz: float):
        """
        Set internal modulation frequency in Hz.
        
        Valid when MOD:SOUR is INT or INT2.
        Typical range: 0.1 Hz – 1 MHz (depends on modulation type).
        """
        if freq_hz <= 0:
            raise ValueError("Modulation frequency must be positive.")
        self.parent.inst.write(f"MOD:FREQ {freq_hz:.6f}")

    def get_frequency(self) -> float:
        """Get internal modulation frequency in Hz."""
